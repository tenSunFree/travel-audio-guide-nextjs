...
function readAll(): Article[] {
  ...
  catch { writeAll(seedArticles); return clone(seedArticles); }
}
...
function assertUniqueSlug(articles: Article[], slug: string, ignoredId?: string) {
...
export const articleRepository: ArticleRepository = {
  ...
  async getById(id: string): Promise<Article | null> {
    await delay(); return readAll().find((a) => a.id === id) ?? null;
  },
  async getPublishedBySlug(slug: string): Promise<Article | null> {
    await delay(); return readAll().find((a) => a.slug === slug && a.status === "published") ?? null;
  },
  async create(values: ArticleFormValues): Promise<Article> {
    await delay();
    const articles = readAll(); assertUniqueSlug(articles, values.slug);
    const timestamp = now();
    const article: Article = {
      id: crypto.randomUUID(), ...formToArticleData(values),
      createdAt: timestamp, updatedAt: timestamp,
      publishedAt: values.status === "published" ? timestamp : null
    };
    writeAll([article, ...articles]); return clone(article);
  },
  async update(id: string, values: ArticleFormValues): Promise<Article> {
    ...
    const article: Article = {
      ...current, ...formToArticleData(values), updatedAt: now(),
      publishedAt: values.status === "published" ? current.publishedAt ?? now() : null
    };
    writeAll(articles.map((a) => a.id === id ? article : a)); return clone(article);
  },
  async duplicate(id: string): Promise<Article> {
    ...
    let slug = `${source.slug}-copy`; let index = 2;
    while (articles.some((a) => a.slug === slug)) slug = `${source.slug}-copy-${index++}`;
    const timestamp = now();
    const copy: Article = { ...source, id: crypto.randomUUID(), title: `${source.title}（副本）`, slug, status: "draft", createdAt: timestamp, updatedAt: timestamp, publishedAt: null };
    writeAll([copy, ...articles]); return clone(copy);
  },
  async remove(id: string): Promise<void> {
    await delay(); writeAll(readAll().filter((a) => a.id !== id));
  },
  exportJson(): string { return JSON.stringify(readAll(), null, 2); },
  importJson(raw: string): number {
    const articles = articleSchema.array().parse(JSON.parse(raw));
    const slugs = new Set(articles.map((a) => a.slug));
    if (slugs.size !== articles.length) throw new Error("匯入資料包含重複網址代稱");
    writeAll(articles); return articles.length;
  }
};
