...
export function ArticleListPage() {
  ...
  const duplicateMutation = useMutation({
    ...
    onSuccess: async (article) => { await refresh(); navigate(`/admin/articles/${article.id}/edit`); }
  });
  ...
  return <section>
    <PageHeader title="文章管理" description={`共 ${articles.length} 篇文章；資料儲存在目前瀏覽器。`} actions={<>
      ...
      <Link className="button primary" to="/admin/articles/new"><Plus size={16}/>新增文章</Link>
    </>}/>
    ...
    <div className="card table-card">
      ...
      <tbody>{filtered.map((article) => <tr key={article.id}>
        ...
        <td><div className="actions-cell">
          <Link title="編輯" to={`/admin/articles/${article.id}/edit`}><Pencil size={17}/></Link>
          <button title="建立副本" onClick={() => duplicateMutation.mutate(article.id)}><Copy size={17}/></button>
          {article.status === "published" && <Link title="公開頁" target="_blank" to={`/articles/${article.slug}`}><ExternalLink size={17}/></Link>}
