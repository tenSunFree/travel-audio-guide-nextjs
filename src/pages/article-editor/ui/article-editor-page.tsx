...
export function ArticleEditorPage() {
  ...
  const form = useForm<ArticleFormValues>({
    ...
    defaultValues: article ? articleToForm(article) : emptyValues,
  });
  useEffect(() => {
        ...
        actions={
          <>
            {articleId && (
              <Link
                className="button"
                to={`/admin/articles/${articleId}/preview`}
              >
                <Eye size={16} />
                預覽
              </Link>
            ...
            {articleId && status === "published" && (
              <Link className="button" target="_blank" to={`/articles/${slug}`}>
                <ExternalLink size={16} />
                公開頁
              </Link>
            )}
            <Link className="button" to="/admin/articles">
              返回列表
            </Link>
          </>
        }
