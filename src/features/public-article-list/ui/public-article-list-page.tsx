...
export function PublicArticleListPage() {
  ...
  if (isLoading)
    return (
      <main className="public-page public-index">
        <p>載入中…</p>
      </main>
    );
  ...
  return (
    <main className="public-page public-index">
      <header>
        <Link href="/admin/articles">CMS 後台</Link>
        <p className="publication">THE DESK JOURNAL</p>
        <h1>Ideas, notes and field reports.</h1>
        <p>由本地 React CMS 發布的文章。</p>
      </header>
      <section className="public-list">
        ...
        {articles.length === 0 && (
          <div className="empty-state">
            <h2>尚無公開文章</h2>
            <Link href="/admin/articles">前往 CMS 建立文章</Link>
          </div>
        )}
      </section>
    </main>
