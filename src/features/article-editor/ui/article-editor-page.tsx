...
import { articleFormSchema, type ArticleFormValues } from "@/shared/api/article.schema";
...
export function ArticleEditorPage({ articleId }: { articleId?: string }) {
  ...
  const form = useForm<ArticleFormValues>({ resolver: zodResolver(articleFormSchema), defaultValues: emptyValues });
  ...
  if (isEditing && isLoading) {
    return <section><PageHeader title="編輯文章" description="載入中…" /></section>;
  }

  return <section>
    <PageHeader title={isEditing ? "編輯文章" : "新增文章"} description="儲存後即可透過公開路由產生文章網頁。" actions={<>
      {articleId && <Link className="button" href={`/admin/articles/${articleId}/preview`}><Eye size={16}/>預覽</Link>}
      {articleId && status === "published" && <Link className="button" target="_blank" href={`/articles/${slug}`}><ExternalLink size={16}/>公開頁</Link>}
      <Link className="button" href="/admin/articles">返回列表</Link>
    </>}/>
