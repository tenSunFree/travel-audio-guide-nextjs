...
export function ArticleEditorPage({ articleId }: { articleId?: string }) {
  ...
  // Previously React Router used a loader to fetch the article before render;
  // since localStorage cannot be read during server-side rendering, we fetch with useQuery after entering the page.
  const { data: article, isLoading } = useQuery({
    ...articleDetailQuery(articleId ?? ""),
    enabled: isEditing,
  });

  useEffect(() => {
    if (isEditing && !isLoading && article === null) notFound();
  }, [isEditing, isLoading, article]);
  ...
  useEffect(() => {
    if (article) form.reset(articleToForm(article));
  }, [article, form]);

  const saveMutation = useMutation({
    mutationFn: (values: ArticleFormValues) =>
      isEditing && articleId
        ? articleRepository.update(articleId, values)
        : articleRepository.create(values),
  ...
  if (isEditing && isLoading) {
    return (
      <section>
        <PageHeader title="編輯文章" description="載入中…" />
      </section>
    );
  }
        ...
        <div className="editor-grid">
          <div className="card form-card">
            <div className="tab-row">
              ...
              <button
                type="button"
                className={activePanel === "seo" ? "active" : ""}
            ...
            {activePanel === "content" ? (
              <>
                ...
                >
                  <input
                    ...
                    placeholder="例如：台北咖啡廳指南"
                  />
                </Field>
                <Field
                  label="公開網址"
                  hint={`/articles/${slug || "your-article-slug"}`}
                ...
                >
                  <input
                    ...
                    placeholder="react, cms, architecture"
                  />
                </Field>
                <Field
                  label="摘要"
                  hint={`${form.watch("excerpt").length}/200`}
              ...
              <>
                <Field
                  label="SEO 標題"
                  hint={`${form.watch("seoTitle").length}/70；留空則使用文章標題`}
                ...
                <Field
                  label="SEO 描述"
                  hint={`${form.watch("seoDescription").length}/160；留空則使用摘要`}
                  error={form.formState.errors.seoDescription?.message}
                >
                  <textarea rows={4} {...form.register("seoDescription")} />
                </Field>
                <div className="seo-preview">
                  ...
                  <span>example.com/articles/{slug || "article-slug"}</span>
                  <p>
            ...
            {saveMutation.error && (
              <div className="alert error">{saveMutation.error.message}</div>
            )}
            {saveMutation.isSuccess && !form.formState.isDirty && (
              <div className="alert success">文章已儲存。</div>
            )}
            <div className="form-actions">
              ...
              >
                <Save size={16} />
                {saveMutation.isPending ? "儲存中…" : "儲存文章"}
              </button>
            </div>
          </div>
          <aside className="card preview-card">
            <div className="preview-label">LIVE PREVIEW</div>
            <h1>{form.watch("title") || "未命名文章"}</h1>
            {form.watch("excerpt") && (
